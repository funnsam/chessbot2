...
pub fn see(game: &Game, m: ChessMove) -> i16 {
    ...
    if let Some(target) = game.board().piece_on(m.get_dest()) {
        ...
        let mut may_xray = game.board().pieces(chess::Piece::Pawn) |
            game.board().pieces(chess::Piece::Bishop) |
            game.board().pieces(chess::Piece::Rook) |
            game.board().pieces(chess::Piece::Queen);
        let mut gain = [0; 32];
        ...
        for d in 1..32 {
            ...
            if (from & may_xray).0 != 0 {
                // TODO: attadef |= xrays
                // 7k/4r3/4q3/8/4Q3/3P1B2/8/K7 b - - 0 1
                // the black rook was covered and opened up by black queen currently not considered

                // println!("xray");
                // game.visualize(may_xray & !attadef);
                attadef |= game.board().pseudo_attacks_to(m.get_dest(), may_xray & !attadef, combined);
            }
